...
public class MazeSpawner : MonoBehaviour
{
    ...
    void SpawnGoalInEndRoomCenter()
    {
        ...
        goal.tag = "Goal";
        goal.name = "MazeGoal"; // Give it a recognizable name

        // Find and setup echolocation system
        EcholocationSystem echoSystem = FindFirstObjectByType<EcholocationSystem>();
        if (echoSystem != null)
        {
            echoSystem.SetGoal(goal.transform);
            //Debug.Log("Echolocation system linked to goal");
        }
        else
        {
            //Debug.LogWarning("No EcholocationSystem found in scene!");
        }
        ...
        //Debug.Log($"Goal and echolocation target spawned at end room center: row {centerRow}, column {centerCol}, world position ({x}, 1, {z})");
    }
